...
function buildSystemPrompt(subscription, schoolFormat = null) {
  ...
  return [
    ...
    premium
      ? [
          "【実習パス版】観察事実・学生自身の考察・明日の観察視点・先生に確認したい点を分け、問い返しとして深めてください。",
    ...
    '出力例の構造: {"headings":["エピソードの整理","気づきの確認","表現の確認","明日の観察","先生への相談"],"sections":["...","...","...","...","..."],"checks":["...","...","..."]}',
  ].join("\n");
}
...
function buildUserPrompt(kind, payload, schoolFormat = null) {
  ...
  return [
    ...
    "5. 先生への相談 — 学校の担当教員に確認したい点を、学生が質問しやすい形で整理する。実習先の指導内容を評価・分析する表現にしない。実習先指導員からの助言は、翌日の観察や学生の理解確認に変換し、実習先への評価・批判にしない。翌日の実習中に意識する内容は『明日の観察』に分け、実習担当教員への相談と混同しない。",
    "",
...
const DEFAULT_DIARY_HEADINGS = ["エピソードの整理", "気づきの確認", "表現の確認", "明日の観察", "先生への相談"];
...
function buildPlanMock(payload, subscription, schoolFormat = null) {
  ...
  return {
    headings,
    sections: [
      `${payload.age}を対象に、${payload.time}程度で行う「${title}」の指導案である。活動内容は学生メモに書かれた範囲で整理し、未定の部分は担当の先生に確認したい。`,
    ...
    checks: premium
      ? ["ねらいが子どもの経験として書けていますか。", "保育所保育指針の5領域との関連は入力内容から確認できますか。", "活動の流れは入力した内容から外れていませんか。", "安全面と個別配慮を担当の先生に確認できますか。"]
